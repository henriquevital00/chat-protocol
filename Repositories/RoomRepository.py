...
class RoomRepository():
    ...
    def findUsersAtRoom(self, id):
        ...
        return (
            RoomUser.select(RoomUser.user_id, User.username, Room.admin_id)
            .join(User, attr='user')
            .switch(RoomUser)
            .join(Room, attr='room')
            .switch(RoomUser)
            .where((Room.id == id)& (RoomUser.isInRoom == 1))
        )
    ...
    def findPendingUsers(self, id):
        ...
        return (
            RoomUser.select(RoomUser.user_id, User.username)
            .join(User, attr="user")
            .switch(RoomUser)
    ...
    def findRoomMessages(self, id):
        return (
            Message.select(User.username, Message.content)
    ...
    def rejectUser(self, user_id, room_id):
        RoomUser.delete().where((RoomUser.user_id == user_id) & (RoomUser.room_id == room_id)).execute()


    def acceptUser(self, user_id, room_id):
        RoomUser.update(isInRoom=True).where((RoomUser.user_id == user_id) & (RoomUser.room_id == room_id)).execute()
